...
def test_controller():
    ...
    return jsonify({
        'message': 'Task controller is working!',
        'total_tasks': len(tasks),
        'tasks': task_data
    })
    
@task_bp.route('/create-test', methods=['POST', 'GET'])
def create_test():
    """Test route to create a task without templates"""
    if request.method == 'GET':
        return '''
        <h1>Test Task Creation</h1>
        <form method="POST">
            <p>Title: <input type="text" name="title" value="Test Task from Controller" required</p>
            <p>Description: <textarea name="description">This is a test task created via controller</textarea></p>
            <p>Priority:
                <select name="priority">
                    <option value="low">Low</option>
                    <option value="medium" selected>Meidum</option>
                    <option vlaue="high">High</option>
                </select>
            </p>
            <p>button type="submit">Create Task</p>
        </form>
        '''
    ...
    if not user:
        return '<h1>Error: No user found. Visit /test first to create a user.</h1>'
    ...
    try:
        db.session.add(task)
        db.session.commit()
        return f'<h1>Success!</h1><p>Task "{title}" created with ID: {task.id}</p>'
